...
from tests.fakes import FakeArchive, FakeMarket
...
def test_missing_daily_leaves_gap_that_quality_reports(tmp_path):
    archive = FakeArchive(YearMonth(2025, 3), date(2025, 3, 9), missing_days={date(2025, 3, 4)})
    market = FakeMarket(["BTCUSDT"])
    store, rep = run(tmp_path, archive, market)
    assert (
        "BTCUSDT-1m-2025-03-04.zip" in rep.series[(Dataset.KLINES, "BTCUSDT", "1m")].missing_files
    )
